...
public class InOutGen extends GenForm
{
	...
	//  Create SQL
        ...
        if (m_DatePromised != null)
        {
        	...
        	sql.append(" AND o.DatePromised <= ").append(DB.TO_DATE(m_DatePromised)).append(")");
        }
...
//		String docActionSelected = (String)docAction.getValue();
		...
		//	Add Parameter - ConsolidateDoc=Y/N
		ip = new MPInstancePara(instance, 40);
		ip.setParameter("ConsolidateDocument", m_ConsolidateDoc);
		if(!ip.save())
		{
			String msg = "No Parameter added";  //  not translated
			info = msg;
		...
		//	Add Parameter - UnconfirmedInOut=Y/N
		ip = new MPInstancePara(instance, 50);
		ip.setParameter("IsUnconfirmedInOut", m_UnconfirmedInOut);
		if(!ip.save())
		{
			String msg = "No Parameter added";  //  not translated
			info = msg;
		...
		//	Add Parameter - MovementDate=x
		if (m_MovementDate != null)
		{
			ip = new MPInstancePara(instance, 60);
			ip.setParameter("MovementDate", m_MovementDate);
			if(!ip.save())
			{
				String msg = "No Parameter added";  //  not translated
				info = msg;
