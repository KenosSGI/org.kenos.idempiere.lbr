...
{
	/**	Process Name				*/
	public static final String PROCESS_NAME = org.compiere.process.StorageCleanup.class.getName();
	...
	protected String doIt () throws Exception
	{
		...
		sql = "SELECT * "
			...
			+ " AND EXISTS (SELECT * FROM M_Product p "
				+ "WHERE s.M_Product_ID=p.M_Product_ID AND p.M_AttributeSet_ID IS NULL)"
		...
		try
		{
			...
			{
				//	Create Movement
				int AD_Org_ID = rs.getInt(MMovement.COLUMNNAME_AD_Org_ID);
				
				if (mh == null || mh.getAD_Org_ID() != AD_Org_ID)
				{
					mh = new MMovement (getCtx(), 0, get_TrxName());
					mh.setAD_Org_ID(AD_Org_ID);
					mh.setC_DocType_ID(p_C_DocType_ID);
					mh.setDescription(getName());
					mh.saveEx();
					
					addBufferLog(mh.getM_Movement_ID(), mh.getMovementDate(), null, "Movimentação: " + mh.getDocumentNo(), mh.get_Table_ID(), mh.getM_Movement_ID());
				}
				
				lines += move (mh, new MStorageOnHand(getCtx(), rs, get_TrxName()));
			}
 		}
		catch (Exception e)
		{
	 ...
	 */
	private int move (MMovement mh, MStorageOnHand target)
	{
