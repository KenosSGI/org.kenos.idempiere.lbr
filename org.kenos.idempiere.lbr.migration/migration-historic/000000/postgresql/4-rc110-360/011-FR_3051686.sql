...
--DROP VIEW ADEMPIERE.LBR_SITIPI;
CREATE OR REPLACE VIEW LBR_SITIPI (AD_CLIENT_ID, LBR_NOTAFISCAL_ID, DATAENTRADA, DATEDOC, ISCANCELLED, CFOP, UF, TAXSTATUS, VALORTOTAL, BASEIPI, ALIQIPI, VALORIPI, ISENTOIPI, OUTROSIPI)
AS
  SELECT nf.AD_Client_ID,
    nf.LBR_NotaFiscal_ID,
    COALESCE(nf.lbr_dateinout,nf.datedoc) AS DataEntrada,
    ...
    nf.IsCancelled,
    COALESCE(nfl.lbr_CFOPName,'0')      AS CFOP,
    COALESCE(lbr_BPRegion,'EX')         AS UF,
    COALESCE(nfl.lbr_TaxStatusIPI,'00') AS TaxStatus,
    SUM( (
    CASE
      WHEN ((nfl.lbr_CFOPName LIKE '3.%'
      OR nfl.lbr_CFOPName LIKE '7.%')
      ...
      THEN COALESCE(nfl.LineTotalAmt, 0)   +                         -- VALOR DA LINHA
        ...
        COALESCE(tcofins.lbr_TaxAmt, 0)    +                         -- VALOR COFINS
        ((COALESCE(nfl.LineTotalAmt, 0)    * (COALESCE(nf.FreightAmt,0) + -- FRETE, SEGURO, SISCOMEX
        COALESCE(nf.lbr_InsuranceAmt, 0)   + 
	COALESCE(nf.lbr_TotalSISCOMEX,0))) / COALESCE(CASE WHEN (nf.TotalLines = 0) THEN 1 ELSE nf.TotalLines END, 1))
      ELSE COALESCE(nfl.LineTotalAmt, 0)   +                         -- VALOR DA LINHA
        COALESCE(tipi.lbr_TaxAmt, 0)       +                         -- VALOR IPI
        ((COALESCE(nfl.LineTotalAmt, 0)    * (COALESCE(nf.FreightAmt,0) + -- FRETE, SEGURO
        COALESCE(nf.lbr_InsuranceAmt, 0))) / COALESCE(CASE WHEN (nf.TotalLines = 0) THEN 1 ELSE nf.TotalLines END, 1))
    ...
    CASE
      ...
      THEN (COALESCE(tipi.lbr_TaxBaseAmt,0) / CASE WHEN (1-(COALESCE(tipi.lbr_TaxBase,0)/100)) = 0 THEN 1 ELSE (1-(COALESCE(tipi.lbr_TaxBase,0)/100)) END) - COALESCE(tipi.lbr_TaxBaseAmt,0)
      ELSE 0
    END) AS isentoIPI,
    SUM(
    CASE
  ...
  GROUP BY nf.AD_Client_ID,
    ...
    nf.IsCancelled,
    COALESCE(nfl.lbr_CFOPName,'0'),
    COALESCE(nfl.lbr_TaxStatusIPI,'00'),
    COALESCE(lbr_BPRegion,'EX');

--DROP VIEW ADEMPIERE.LBR_SITIPI_V;    
CREATE OR REPLACE VIEW LBR_SITIPI_V (AD_CLIENT_ID, LBR_NOTAFISCAL_ID, DATAENTRADA, DATEDOC, ISCANCELLED, CFOP, UF, VALORTOTAL, BASEIPI, ALIQIPI, VALORIPI, ISENTOIPI, OUTROSIPI)
AS
  SELECT AD_Client_ID,
    LBR_NotaFiscal_ID,
    DataEntrada,
...
--DROP VIEW ADEMPIERE.LBR_SITIPI_TIPO_V;
CREATE OR REPLACE VIEW LBR_SITIPI_TIPO_V (AD_CLIENT_ID, LBR_NOTAFISCAL_ID, ISCANCELLED, CFOP, VALORCONTABIL, BASE, ALIQ, TIPOIMPOSTO, VALORIMPOSTO)
AS
  SELECT AD_Client_ID,
    LBR_NotaFiscal_ID,
    IsCancelled,
