-- View: adempiere.lbr_factfiscalbase

DROP VIEW adempiere.lbr_factfiscalbase;
 ...
 SELECT nf.ad_client_id,
    ...
    round(COALESCE(( SELECT sum(abs(lbr_nflinetax.lbr_taxamt)) AS sum
          ...
          WHERE lbr_nflinetax.lbr_taxrate < 0::numeric AND (lbr_nflinetax.lbr_notafiscalline_id IN ( SELECT lbr_notafiscalline.lbr_notafiscalline_id
                   FROM adempiere.lbr_notafiscalline
    ...
    round(COALESCE(tot_icms.lbr_taxbaseamt, 0::numeric), 2) AS icms_nftaxbaseamt,
    round(COALESCE(tot_icms.lbr_taxamt, 0::numeric), 2) AS icms_nftaxamt,
        CASE
    ...
    COALESCE(nfl.lbr_productsource, '0'::bpchar)::text || COALESCE(icms.lbr_taxstatus, '00'::character varying)::text AS icms_taxstatus,
    ...
    nf.dateacct
   FROM adempiere.lbr_notafiscalline nfl
     JOIN adempiere.lbr_notafiscal nf ON nf.lbr_notafiscal_id = nfl.lbr_notafiscal_id
     JOIN adempiere.ad_orginfo oi ON oi.ad_org_id = nf.ad_org_id
     LEFT JOIN adempiere.c_invoice i ON nf.c_invoice_id = i.c_invoice_id
     LEFT JOIN adempiere.c_doctype dt ON nf.c_doctypetarget_id = dt.c_doctype_id
     LEFT JOIN adempiere.c_doctype dti ON i.c_doctypetarget_id = dti.c_doctype_id
     LEFT JOIN adempiere.c_invoiceline il ON nfl.c_invoiceline_id = il.c_invoiceline_id
     LEFT JOIN adempiere.m_product p ON p.m_product_id = nfl.m_product_id
     LEFT JOIN adempiere.c_uom uomp ON uomp.c_uom_id = p.c_uom_id
     LEFT JOIN adempiere.lbr_ncm ncmp ON ncmp.lbr_ncm_id = p.lbr_ncm_id
     LEFT JOIN adempiere.c_location orgloc ON orgloc.c_location_id = nf.org_location_id
     LEFT JOIN adempiere.c_city orgcity ON orgcity.c_city_id = orgloc.c_city_id
     LEFT JOIN adempiere.c_country orgcountry ON orgcountry.c_country_id = orgloc.c_country_id
     LEFT JOIN adempiere.c_bpartner_location bpl ON bpl.c_bpartner_location_id = nf.c_bpartner_location_id
     LEFT JOIN adempiere.c_location bploc ON bploc.c_location_id = bpl.c_location_id
     LEFT JOIN adempiere.c_city bpcity ON bpcity.c_city_id = bploc.c_city_id
     LEFT JOIN adempiere.c_country bpcountry ON bpcountry.c_country_id = bploc.c_country_id
     LEFT JOIN adempiere.lbr_cfop cfop ON nfl.lbr_cfop_id = cfop.lbr_cfop_id
     LEFT JOIN adempiere.c_uom uom ON uom.c_uom_id = nfl.c_uom_id
     LEFT JOIN adempiere.factfiscal_nflinetax_v icms ON nfl.lbr_notafiscalline_id = icms.lbr_notafiscalline_id AND icms.lbr_taxname::text = 'ICMS'::text
     LEFT JOIN adempiere.factfiscal_nflinetax_v pis ON nfl.lbr_notafiscalline_id = pis.lbr_notafiscalline_id AND pis.lbr_taxname::text = 'PIS'::text
     LEFT JOIN adempiere.factfiscal_nflinetax_v cofins ON nfl.lbr_notafiscalline_id = cofins.lbr_notafiscalline_id AND cofins.lbr_taxname::text = 'COFINS'::text
     LEFT JOIN adempiere.factfiscal_nflinetax_v icmsst ON nfl.lbr_notafiscalline_id = icmsst.lbr_notafiscalline_id AND icmsst.lbr_taxname::text = 'ICMSST'::text AND (icmsst.lbr_taxstatus::text = ANY (ARRAY['10'::character varying::text, '30'::character varying::text, '70'::character varying::text]))
     LEFT JOIN adempiere.factfiscal_nflinetax_v ipi ON nfl.lbr_notafiscalline_id = ipi.lbr_notafiscalline_id AND ipi.lbr_taxname::text = 'IPI'::text
     LEFT JOIN adempiere.factfiscal_nflinetax_v ii ON nfl.lbr_notafiscalline_id = ii.lbr_notafiscalline_id AND ii.lbr_taxname::text = 'II'::text
     LEFT JOIN adempiere.lbr_nftax tot_icms ON nf.lbr_notafiscal_id = tot_icms.lbr_notafiscal_id AND tot_icms.lbr_taxgroup_id = (( SELECT lbr_taxgroup.lbr_taxgroup_id
           FROM adempiere.lbr_taxgroup
          WHERE lbr_taxgroup.name::text ~~ 'ICMS'::text))
     LEFT JOIN adempiere.lbr_nftax tot_pis ON nfl.lbr_notafiscal_id = tot_pis.lbr_notafiscal_id AND tot_pis.lbr_taxgroup_id = (( SELECT lbr_taxgroup.lbr_taxgroup_id
           FROM adempiere.lbr_taxgroup
          WHERE lbr_taxgroup.name::text ~~ 'PIS'::text))
     LEFT JOIN adempiere.lbr_nftax tot_cofins ON nfl.lbr_notafiscal_id = tot_cofins.lbr_notafiscal_id AND tot_cofins.lbr_taxgroup_id = (( SELECT lbr_taxgroup.lbr_taxgroup_id
           FROM adempiere.lbr_taxgroup
          WHERE lbr_taxgroup.name::text ~~ 'COFINS'::text))
     LEFT JOIN adempiere.lbr_nftax tot_icmsst ON nfl.lbr_notafiscal_id = tot_icmsst.lbr_notafiscal_id AND tot_icmsst.lbr_taxgroup_id = (( SELECT lbr_taxgroup.lbr_taxgroup_id
           FROM adempiere.lbr_taxgroup
          WHERE lbr_taxgroup.name::text ~~ 'ICMSST'::text))
     LEFT JOIN adempiere.lbr_nftax tot_ipi ON nfl.lbr_notafiscal_id = tot_ipi.lbr_notafiscal_id AND tot_ipi.lbr_taxgroup_id = (( SELECT lbr_taxgroup.lbr_taxgroup_id
           FROM adempiere.lbr_taxgroup
  ...
  WHERE
        ...
        CASE
            ...
            ELSE true
        END AND
        (nf.lbr_nfeenv is null or nf.lbr_nfeenv='1')
UNION
 SELECT bp.ad_client_id,
   ...
   FROM adempiere.c_bpartner bp
     LEFT JOIN adempiere.c_bpartner_location bpl ON bpl.c_bpartner_id = bp.c_bpartner_id
     LEFT JOIN adempiere.c_location bploc ON bploc.c_location_id = bpl.c_location_id
     LEFT JOIN adempiere.c_city bpcity ON bpcity.c_city_id = bploc.c_city_id
     LEFT JOIN adempiere.c_country bpcountry ON bpcountry.c_country_id = bploc.c_country_id
     LEFT JOIN adempiere.c_region r ON r.c_region_id = bploc.c_region_id
     JOIN adempiere.lbr_salescardtotal card ON card.c_bpartner_id = bp.c_bpartner_id
     LEFT JOIN adempiere.c_period p ON p.c_period_id = card.c_period_id;

ALTER TABLE adempiere.lbr_factfiscalbase
    OWNER TO adempiere;

-- 25 de mar de 2021 17:22:56 BRT
SELECT Register_Migration_Script ('202103241732_DateAcct_FactFiscalBase.sql') FROM DUAL
;