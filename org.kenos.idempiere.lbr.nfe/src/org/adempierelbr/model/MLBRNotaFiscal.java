...
public class MLBRNotaFiscal extends X_LBR_NotaFiscal implements DocAction, DocOptions
{
	...
	{		
		...
		if (sendMail)
		{
			...
			try
			{
				MLBRNFConfig nfConfig = MLBRNFConfig.get(nf.getAD_Org_ID());
				
				if (nfConfig == null || nfConfig.getSendEMail() == null								//	Compatibility with past 
						|| nfConfig.getSendEMail().equals(MLBRNFConfig.SENDEMAIL_SendImmediately))
		...
		if (DOCSTATUS_Completed.equals(getDocStatus()) && LBR_ISOWNDOCUMENT_IssuedByUsOwnDocument.equals(getlbr_IsOwnDocument()))
		{
					...
					&& TextUtil.match(getlbr_NFModel(), LBR_NFMODEL_NotaFiscalEletrônica, LBR_NFMODEL_NotaFiscalDeConsumidorEletrônica))
			{
				...
				catch (Exception e)
				{
					...
					&& getlbr_NFENo() != null && getlbr_NFeProt() != null)
			{
				INFSe infSe = NFSeUtil.get (this);
				//
				if (infSe != null)
				...
				&& TextUtil.match(getDocStatus(), DOCSTATUS_Drafted, DOCSTATUS_InProgress, DOCSTATUS_Invalid)) || !islbr_IsOwnDocument())
		{
			...
			else
			{
				try
				{
					...
					//
					TRetInutNFe.InfInut ret = ProcInutNF.invalidateNF (p_ctx, getAD_Org_ID(), getlbr_CNPJ(), 
					...
					if (MLBRNotaFiscal.LBR_NFESTATUS_102_InutilizaçãoDeNúmeroHomologado.equals(ret.getCStat())	//	OK
							...
							|| MLBRNotaFiscal.LBR_NFESTATUS_256_RejeiçãoUmaNF_EDaFaixaJáEstáInutilizadaNaBaseDeDadosDaSEFAZ.equals(ret.getCStat()))
					{
						setDocAction (DOCACTION_None);
	...
	public void calculateWeight ()
	{
		...
		for (MLBRNotaFiscalLine nfl : getLines())
		{
			if (nfl.getM_Product_ID() > 0)
			{
				...
				BigDecimal prodWeight = nfl.getM_Product ().getWeight ();
				BigDecimal packWeight = (BigDecimal)product.get_Value("LBR_PackingWeight");
				BigDecimal qty = nfl.getQty ();
				
				//	In case product weight is null, skip
				if (prodWeight == null)
					return;
