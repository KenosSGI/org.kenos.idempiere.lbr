...
public class ProcImportIBPTax extends SvrProcess
{
	...
	/** IBPT API Key	*/
	private String p_LBR_IBPT_API_Key = "";
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("LBR_IBPT_API_Key"))
				p_LBR_IBPT_API_Key = para[i].getParameter().toString();
			else
	...
	{	
		...
		if (p_ImportFromAPI)
		{
			...
			//
			String apiKey = p_LBR_IBPT_API_Key;
			...
			{	
				...
				if (!apiKey.isEmpty())
				{
					//	If Key not Configurate
					if (nfConfig.getLBR_IBPT_API_Key() == null || nfConfig.getLBR_IBPT_API_Key().isEmpty())
					{
						nfConfig.setLBR_IBPT_API_Key(apiKey);
						nfConfig.save();
					}
				}
				else
					apiKey = nfConfig.getLBR_IBPT_API_Key();
			}	
