 ...
 * @since 1.0.0
 * 
 *        Added the ability to create new records. by @contributor Claudemir
 *        Todo Bom <claudemir@todobom.com>
 * 
 */

@SuppressWarnings({ "rawtypes", "unchecked" })
public class FixedFormatManager implements com.ancientprogramming.fixedformat4j.format.FixedFormatManager {
	...
	public <T> T load(Class<T> fixedFormatRecordClass) {
		...
		for (Method method : allMethods) {
			...
			Fields fieldsAnnotation = method.getAnnotation(Fields.class);
			if (fieldAnnotation != null) {
				initializeFieldData(fixedFormatRecordClass, instance, foundData, methodClass, method, methodName,
						fieldAnnotation);
			} else if (fieldsAnnotation != null) {
				// assert that the fields annotation contains minimum one field anno
				if (fieldsAnnotation.value() == null || fieldsAnnotation.value().length == 0) {
	...
	private <T> void initializeFieldData(Class<T> fixedFormatRecordClass, T instance, HashMap<String, Object> foundData,
			HashMap<String, Class<?>> methodClass, Method method, String methodName, Field fieldAnnotation) {
		// Object loadedData = readDataAccordingFieldAnnotation(fixedFormatRecordClass,
		// data, method, fieldAnnotation);
					...
					try {
						if (datatype.equals(java.util.Date.class)) {
							loadedData = null;
								...
								|| datatype.equals(double.class) || datatype.equals(BigDecimal.class)) {
							loadedData = 0;
						} else if (datatype.equals(Double.class)) {
							loadedData = Double.valueOf(0);
						} else {
							loadedData = datatype.getDeclaredConstructor().newInstance();
						}
					} catch (Exception e1) {
	...
	private FixedFormatPatternData getFixedFormatPatternData(FixedFormatPattern annotation) {
		...
		if (annotation != null) {
			...
			result = new FixedFormatPatternData("ddMMyy");
		}
		return result;
	}
	...
	private FixedFormatDecimalData getFixedFormatDecimalData(FixedFormatDecimal annotation) {
		...
		if (annotation != null) {
			...
			// CNAB 400 Default
			result = new FixedFormatDecimalData(2, false, '.', RoundingMode.HALF_UP);
		}
		return result;
	}
