...
public abstract class CreateFromInvoice extends CreateFrom
{
	...
	protected ArrayList<KeyNamePair> loadShipmentData (int C_BPartner_ID, int C_Order_ID)
	{
		...
		StringBuffer sql = new StringBuffer("SELECT s.M_InOut_ID,").append(display)
			...
			if(!isSOTrx)
			{
				...
				if (C_Order_ID > 0)
				{
					sql.append(" AND sl.C_OrderLine_ID IN (SELECT ol.C_OrderLine_ID FROM C_OrderLine ol ")
						.append(" WHERE ol.C_Order_ID=").append(C_Order_ID).append(") ");
				}
				sql.append(" GROUP BY sl.M_InOut_ID,sl.MovementQty,mi.M_InOutLine_ID"
	 ...
	 */
	protected Vector<Vector<Object>> getShipmentData(int M_InOut_ID, int C_Order_ID)
	{
		...
		if (C_Order_ID > 0)
		{
			sql.append(" AND l.C_OrderLine_ID IN (SELECT ol.C_OrderLine_ID FROM C_OrderLine ol ")
				.append(" WHERE ol.C_Order_ID=").append(C_Order_ID).append(") ");
		}
		
		sql.append("GROUP BY l.MovementQty, l.QtyEntered/l.MovementQty, "
