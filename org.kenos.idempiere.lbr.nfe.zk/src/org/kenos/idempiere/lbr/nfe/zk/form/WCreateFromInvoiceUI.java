...
public class WCreateFromInvoiceUI extends CreateFromInvoice implements EventListener<Event>, ValueChangeListener
{
	...
	public void onEvent(Event e) throws Exception
	{
		...
		if (e.getTarget().equals(orderField))
		{
			...
			initBPShipmentDetails(C_BPartner_ID, C_Order_ID);
		}
		//  Shipment
		else if (e.getTarget().equals(shipmentField))
		{
			...
			if (li != null && li.getValue() != null)
				C_Order_ID = ((Integer) li.getValue()).intValue();
			rmaField.setSelectedIndex(-1);
			loadShipment(M_InOut_ID, C_Order_ID);
		}
		//  RMA
	...
	{
		initBPShipmentDetails(C_BPartner_ID, -1);
	 ...
	 * @param C_BPartner_ID
	 */
	private void initBPShipmentDetails(int C_BPartner_ID, int C_Order_ID)
	{
		if (log.isLoggable(Level.CONFIG)) log.config("C_BPartner_ID" + C_BPartner_ID);
		...
		ArrayList<KeyNamePair> list = loadShipmentData(C_BPartner_ID, C_Order_ID);
		for(KeyNamePair knp : list)
	...
	protected void loadShipment (int M_InOut_ID, int C_Order_ID)
	{
		loadTableOIS(getShipmentData(M_InOut_ID, C_Order_ID));
	}
