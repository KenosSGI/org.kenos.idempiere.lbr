...
import org.kenos.idempiere.lbr.base.form.Match;
	...
	implements IFormController, EventListener<Event>, WTableModelListener
{
	...
	private Listbox org = ListboxFactory.newDropdownListbox();
	...
	private void dynInit()
	{
		...
		for (MOrg o : MOrg.getOfClient(MClient.get(Env.getCtx())))
		{
			org.addItem(new KeyNamePair(o.getAD_Org_ID(), o.getName()));
			if (m_AD_Org_ID == o.getAD_Org_ID())
				org.setSelectedIndex(org.getItemCount()-1);
		}
	...
	public void onEvent (Event e)
	{
		...
		//	Update org
		if (e.getTarget() == org)
			m_AD_Org_ID = (Integer) org.getSelectedItem().getValue();
		...
		{
			//cmd_search();
			xMatchedTable = (WListbox)cmd_search(xMatchedTable, matchFrom.getSelectedIndex(), (String)matchTo.getSelectedItem().getLabel(), product, vendor, from, to, m_AD_Org_ID, matchMode.getSelectedIndex() == MODE_MATCHED);
		...
		else if (e.getTarget() == bProcess || e.getTarget() == org)
		{
//			cmd_process();
	...
	private void cmd_searchTo()
	{
		...
		else
		{
			...
			xMatchedToTable = (WListbox) cmd_searchTo(xMatchedTable, xMatchedToTable, displayString, matchToType, m_AD_Org_ID, sameBPartner.isSelected(), sameProduct.isSelected(), sameQty.isSelected(), matchMode.getSelectedIndex() == MODE_MATCHED);
		}
