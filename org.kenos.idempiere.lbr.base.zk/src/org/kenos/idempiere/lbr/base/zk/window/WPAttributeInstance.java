 ...
 *****************************************************************************/
package org.kenos.idempiere.lbr.base.zk.window;
...
public class WPAttributeInstance extends Window implements EventListener<Event> 
{
	...
	private Checkbox positiveOnly = new Checkbox();
	private Checkbox sameWarehouse = new Checkbox();
	private Textbox descBox = new Textbox();
	private Button filterBtn = new Button(Msg.getMsg(Env.getCtx(), "Execure Query"));
	//
	...
	private void init() throws Exception
	{
		...
		positiveOnly.setLabel(Msg.getMsg(Env.getCtx(), "QtyAvailable"));
		String msg = Msg.getMsg(Env.getCtx(), "FromSameWarehouseOnly");
		if (msg.indexOf("\n") > 0)
			msg = msg.substring(0, msg.indexOf("\n"));
		sameWarehouse.setLabel(msg);
		filterBtn.addEventListener(Events.ON_CLICK, this);
		descBox.addEventListener(Events.ON_OK, this);

		this.appendChild(mainLayout);
		
		//	Default
		sameWarehouse.setSelected(true);
		positiveOnly.setSelected(true);
		...
		positiveOnly.addEventListener(Events.ON_CHECK, this);
		sameWarehouse.addEventListener(Events.ON_CHECK, this);
		
		//	Serial Number box
		box.appendChild(descBox);
		box.appendChild(filterBtn);
		box.appendChild(positiveOnly);
		box.appendChild(sameWarehouse);
	...
	private String	m_sqlAvailable = " AND (s.QtyOnHand<>0)";
	private String	m_sqlMinLife = "";
	private final String	m_sqlFilterASI = " AND asi.Description LIKE ? ";
	...
	private void refresh()
	{
		...
		//	Include On Hand Positive filter
		if (positiveOnly.isChecked())
			sql += m_sqlAvailable;
		//	Include Show All and MinLife Filters
		else if (!showAll.isChecked())
		{
			sql += m_sqlNonZero;
		...
		try
		{
			...
			if (m_M_Warehouse_ID != 0 && sameWarehouse.isChecked())
	...
	public void onEvent(Event e) throws Exception 
	{
		...
		else if (e.getTarget() == showAll || e.getTarget() == sameWarehouse || e.getTarget() == positiveOnly 
				|| e.getTarget() == filterBtn || e.getTarget() == descBox)
		{
			if (showAll.isChecked())
				positiveOnly.setChecked(false);
			refresh();
		}
