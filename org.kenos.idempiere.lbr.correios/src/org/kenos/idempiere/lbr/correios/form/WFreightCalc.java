...
import org.kenos.idempiere.lbr.correios.api.Correios;
import org.kenos.idempiere.lbr.correios.api.InterfaceCorreios;
...
public class WFreightCalc extends ADForm implements IFormController, EventListener<Event> {
	...
	private static final String VALOR_DECLARADO = "019";
	...
	InterfaceCorreios correios = null;
	...
	public void onEvent(Event e) throws Exception {
		...
		if (eventName.equals(Events.ON_CHANGE)) {
			...
			// Validate the Weight
				...
				if ("1".equals(f_format.getSelectedItem().toString())
					...
					Messagebox.show(
							"Valor não permitido. Peso mínimo é: " + WEIGHT_MIN.stripTrailingZeros().toPlainString());
		...
		} else if (eventName.equals(Events.ON_CLICK) || eventName.equals(Events.ON_DOUBLE_CLICK)
				|| eventName.equals(Events.ON_OK)) {
			// Cancel button
			...
			// Query Button
				...
				if (TextUtil.toNumeric(f_zipFrom.getText()).length() != 8
						|| TextUtil.toNumeric(f_zipTo.getText()).length() != 8)
					Messagebox.show("CEP Inválido, confira o CEP digitado");
				else {
					getFreight();
			...
			// OK button
				...
				if (getGridTab() != null) {
					if (getGridTab().getValueAsBoolean(MOrder.COLUMNNAME_Processed)) {
						Messagebox.show(
								"Pedido já processado, não é possível atualizar o frete. Re-abra o pedido primeiramente");
						return;
					}
					...
					if (category != null) {
						...
						BigDecimal price = new BigDecimal(
								String.valueOf(priceCell.getLabel()).replace(".", "").replace(",", "."));

						if (price.signum() == 1) {
							MFreightCategory fc = MFreightCategory.getByValue(Env.getCtx(),
									TextUtil.lPad(codeCell.getTooltip(), 5), null);
							if (fc == null) {
								Messagebox
										.show("Não foi possível atualizar o pedido, categoria de frete não encontrada");
								return;
							}
							//
							...
							Messagebox.show(
									"Não foi possível atualizar o pedido, preço de frete inválido ou não selecionado");
					} else
	...
	private void updatePackageWeight() {
		for (X_M_ShipperPackaging p : listPack) {
			if (p.getM_ShipperPackagingCfg().getValue().equals(f_format.getSelectedItem().toString())) {
	...
	protected void initForm() {
		...
		List<X_M_Freight> list = new Query(Env.getCtx(), X_M_Freight.Table_Name,
				X_M_Freight.COLUMNNAME_M_Shipper_ID + "=?", null).setParameters(m_shipper.getM_Shipper_ID()).list();
		for (X_M_Freight freight : list)
			m_freights.add(new ValueNamePair(freight.getM_FreightCategory().getValue(),
					freight.getM_FreightCategory().getName()));
		if (m_freights.size() <= 0)
			throw new AdempiereException("Transportadora dos correios sem configuração de fretes disponíveis");

		listPack = new Query(Env.getCtx(), X_M_ShipperPackaging.Table_Name,
				X_M_ShipperPackaging.COLUMNNAME_M_Shipper_ID + "=?", null).setParameters(m_shipper.getM_Shipper_ID())
						.list();
		for (X_M_ShipperPackaging pack : listPack)
			m_formats.add(new KeyNamePair(Integer.parseInt(pack.getM_ShipperPackagingCfg().getValue()),
					pack.getM_ShipperPackagingCfg().getName()));
		if (m_formats.size() <= 0)
	...
	private void getFreight() throws Exception {
		...
		/**
		 * Seu código administrativo junto à ECT. O código está disponível no corpo do
		 * contrato firmado com os Correios.
		 * 
		 * Obrigatório para clientes com contrato. Para clientes sem contrato informar o
		 * parâmetro vazio.
		 */
		String nCdEmpresa72 = "";
		...
		/**
		 * Senha para acesso ao serviço, associada ao seu código administrativo. A senha
		 * inicial corresponde aos 8 primeiros dígitos do CNPJ informado no contrato. A
		 * qualquer momento, é possível alterar a senha no endereço
		...
		/**
		 * CEP de Origem sem hífen.Exemplo: 05311900 Obrigatório
		 */
		String sCepOrigem75 = TextUtil.toNumeric(f_zipFrom.getText());

		/**
		 * CEP de Destino sem hífen Obrigatório
		 */
		 ...
		 * quilogramas. Se o formato for Envelope, o valor máximo permitido será 1kg.
		 * Obrigatório
		 */
		String nVlPeso77 = TextUtil.toNumeric(f_weightTotal.getValue(), 2).replace(".", ",");

		/**
		 * Formato da encomenda (incluindo embalagem). Valores possíveis: 1, 2 ou 3
		 * 
		 * 1 – Formato caixa/pacote 2 – Formato rolo/prisma 3 - Envelope Obrigatório
		 */
		int nCdFormato78 = f_format.getSelectedItem().toKeyNamePair().getKey();

		/**
		 * Comprimento da encomenda (incluindo embalagem), em centímetros. Obrigatório
		 */
		BigDecimal nVlComprimento79 = f_length.getValue();

		/**
		 * Altura da encomenda (incluindo embalagem), em centímetros. Se o formato for
		 * envelope, informar zero (0). Obrigatório
		 */
		BigDecimal nVlAltura80 = f_height.getValue();

		/**
		 * Largura da encomenda (incluindo embalagem), em centímetros. Obrigatório
		 */
		BigDecimal nVlLargura81 = f_width.getValue();

		/**
		 * Diâmetro da encomenda (incluindo embalagem), em centímetros. Obrigatório
		 */
		 ...
		 * recebimento. Valores possíveis: S ou N
		 * 
		 * S – Sim N – Não Obrigatório. Se não optar pelo serviço informar ‘N’.
		 */
		String sCdAvisoRecebimento85 = cb_confirmReceipt.isSelected() ? "S" : "N";

		/**
		 * Data do cálculo Obrigatório
		 */
		String sDtCalculo86 = TextUtil.timeToString(f_date.getValue(), "yyyy-MM-dd");
...
//		lotePrazoNacional.setIdLote("1");
		...
		for (ValueNamePair vp : m_freights) {
			...
			List<String> servicoAdicionais = new ArrayList<String>();

			ParamPrazoNacional prazoNacional = new ParamPrazoNacional();

			PrecoNacionalParam precoNacional = new PrecoNacionalParam();

			prazoNacional.setCoProduto(nCdServico74);
			prazoNacional.setDataPostagem(sDtCalculo86);
			prazoNacional.setCepDestino(sCepDestino76);
			prazoNacional.setCepOrigem(sCepOrigem75);
			prazoNacional.setNuRequisicao("1");

			precoNacional.setCoProduto(nCdServico74);
			precoNacional.setNuRequisicao("1");
			precoNacional.setCepOrigem(sCepOrigem75);
			precoNacional.setCepDestino(sCepDestino76);
			precoNacional.setPsObjeto(nVlPeso77.replace(",", "."));
			precoNacional.setTpObjeto(Integer.toString(nCdFormato78));
			precoNacional.setComprimento(nVlComprimento79.toString());
			precoNacional.setLargura(nVlLargura81.toString());
			precoNacional.setAltura(nVlAltura80.toString());
			precoNacional.setDiametro(nVlDiametro82.toString());

			if (nVlValorDeclarado84.intValue() > 0) {
				precoNacional.setVlDeclarado(nVlValorDeclarado84.toString());
				servicoAdicionais.add(VALOR_DECLARADO);
			}
			if (sCdMaoPropria83.equals("S")) {
				servicoAdicionais.add(MAO_PROPRIA);
			}
			if (sCdAvisoRecebimento85.equals("S")) {
				servicoAdicionais.add(AVISO_RECEBIMENTO);
			}

			if (servicoAdicionais.size() > 0) {
				precoNacional.setServicosAdicionais(servicoAdicionais);
			}

			precoNacional.setCriterios(new ArrayList<String>());
			precoNacional.setDtEvento(sDtCalculo86);
			precoNacional.setDtArmazenagem(null);

			PrecoPrazo precoPrazo = correios.calcPrazoPreco(precoNacional);

			ListItem item = new ListItem();
			miniTable.appendChild(item);

			ListCell cell = new ListCell(parseCode(Integer.parseInt(precoPrazo.getCodigoProduto())));
			cell.setTooltip(precoPrazo.getCodigoProduto());

			item.appendChild(cell);
			item.appendChild(new ListCell(precoPrazo.getValorFinal()));
			item.appendChild(new ListCell(precoPrazo.getValorMaoPropria()));
			item.appendChild(new ListCell(precoPrazo.getValorRecebimento()));
			item.appendChild(new ListCell(precoPrazo.getValorSeguro()));
			item.appendChild(new ListCell(precoPrazo.getValorProduto()));
			item.appendChild(new ListCell(String.valueOf(precoPrazo.getPrazo() + " dias")));
...
//		l_warning.setText(warningMsg);
	...
	/**
	 * When setting process info, it's possible the fill some data automatically,
	 * like ZIP codes and weight
	 */
	@Override
	public void setProcessInfo(ProcessInfo pi) {
		...
		if (pi != null && pi.getRecord_ID() > 0 && pi.getTable_ID() == MOrder.Table_ID) {
			...
			if (order.getC_BPartner_Location_ID() > 0
					&& order.getC_BPartner_Location().getC_Location().getPostal() != null)
